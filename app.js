...
// Athkar Audio Elements
const athkarPlayBtn = document.getElementById('athkarPlayBtn');
const athkarAudioStatus = document.getElementById('athkarAudioStatus');
const athkarAudioElement = document.getElementById('athkarAudioElement');

let activeAthkarTab = 'sabah';

// High-quality Alafasy audio files for full Athkar sets
const ATHKAR_AUDIO_URLS = {
  sabah: "https://server11.mp3quran.net/download/a_alafasi/adhkar/01.mp3", // أذكار الصباح
  massa: "https://server11.mp3quran.net/download/a_alafasi/adhkar/02.mp3", // أذكار المساء
  quran: "https://server11.mp3quran.net/download/a_alafasi/adhkar/05.mp3"  // أدعية واستغفار
};
...
function renderAthkarList(tab) {
  ...
  // Update UI play button text depending on active tab
  const tabNames = { sabah: "أذكار الصباح", massa: "أذكار المساء", quran: "الأدعية والاستغفار" };
  if (athkarPlayBtn) {
    athkarPlayBtn.textContent = `🔊 تشغيل ${tabNames[tab]} بالصوت`;
    athkarAudioStatus.textContent = "بصوت الشيخ مشاري العفاسي";
  }
  
  // Stop current Athkar audio if tab changes
  athkarAudioElement.pause();
  athkarAudioElement.src = "";
...
// Play/Pause Athkar Audio
if (athkarPlayBtn) {
  athkarPlayBtn.addEventListener('click', () => {
    // 1. Pause Quran recitation audio to prevent overlap
    audioElement.pause();
    isPlaying = false;
    playPauseBtn.textContent = '▶';

    // 2. Play / Pause Athkar audio
    if (!athkarAudioElement.src || athkarAudioElement.src === "" || athkarAudioElement.src !== ATHKAR_AUDIO_URLS[activeAthkarTab]) {
      athkarAudioElement.src = ATHKAR_AUDIO_URLS[activeAthkarTab];
      athkarAudioElement.play().then(() => {
        athkarPlayBtn.textContent = "⏸ إيقاف الصوت مؤقتاً";
        athkarAudioStatus.textContent = "جارٍ التشغيل الآن بصوت الشيخ مشاري العفاسي...";
      }).catch(console.error);
    } else {
      if (athkarAudioElement.paused) {
        athkarAudioElement.play().then(() => {
          athkarPlayBtn.textContent = "⏸ إيقاف الصوت مؤقتاً";
          athkarAudioStatus.textContent = "جارٍ التشغيل الآن بصوت الشيخ مشاري العفاسي...";
        }).catch(console.error);
        ...
        athkarAudioElement.pause();
        const tabNames = { sabah: "أذكار الصباح", massa: "أذكار المساء", quran: "الأدعية والاستغفار" };
        athkarPlayBtn.textContent = `🔊 تشغيل ${tabNames[activeAthkarTab]} بالصوت`;
        athkarAudioStatus.textContent = "بصوت الشيخ مشاري العفاسي (متوقف)";
      }
    }
  });
}
